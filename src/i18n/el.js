const el = {
    translations: {
        'language': 'Γλώσσα',
        'theme': 'Σκούρο θέμα',
        'header_title_start': 'Δες που βρίσκεται',
        'header_title_continue_recycle': 'το πλησιέστερο σήμειο ανακύκλωσης.',
        'header_title_continue_water': 'ο πλησιέστερος σταθμός νερού.',
        'header_title_continue_bus': 'η πλησιέστερη στάση λεωφορείου.',
        'header_location': 'Σύρος, Κυκλάδες, Ελλάδα',
        'input_placeholder': 'Αναζήτησε μια τοποθεσία..',
        'categories_title': 'Θέλω να ανακυκλώσω..',
        'map_title': 'Χάρτης',
        'en': 'Αγγλικά',
        'el': 'Ελληνικά',
        'quick_results': 'Γρήγορα αποτελέσματα',
        'results_based': 'Αποτελέσματα βασισμένα σε: ',
        'no_results': 'Δεν βρέθηκαν αποτελέσματα'
    }
  }
  
  export default el;